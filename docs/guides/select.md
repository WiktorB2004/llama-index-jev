...
[`JevSingleSelector`](../api/selectors.md#llama_index.selectors.jev.JevSingleSelector) and [`JevMultiSelector`](../api/selectors.md#llama_index.selectors.jev.JevMultiSelector) are LlamaIndex `BaseSelector`s: typed tool / query-engine choice with TypeSafe Jev, without an LLM-as-judge.
...
Duplicate or empty tool names fall back to `option_{i}` so Choice keys stay unique.
